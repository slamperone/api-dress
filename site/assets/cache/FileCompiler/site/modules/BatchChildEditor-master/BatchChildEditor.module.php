...
class BatchChildEditor extends \ProcessWire\WireData implements \ProcessWire\Module, \ProcessWire\ConfigurableModule {
   ...
   /**
    ...
    static public function getDefaultData() {
            return array(
                "systemFields" => array(
                    'id' => \ProcessWire\__('ID'),
                    'name' => \ProcessWire\__('Name (from URL)'),
                    'path' => \ProcessWire\__('Path'),
                    'url' => \ProcessWire\__('URL'),
                    'status' => \ProcessWire\__('Status'),
                    'created' => \ProcessWire\__('Date Created'),
                    'modified' => \ProcessWire\__('Date Last Modified'),
                    'createdUser.id' => \ProcessWire\__('Created by User: ID'),
                    'createdUser.name' => \ProcessWire\__('Created by User: Name'),
                    'modifiedUser.id' => \ProcessWire\__('Modified by User: ID'),
                    'modifiedUser.name' => \ProcessWire\__('Modified by User: Name'),
                    'parent_id' => \ProcessWire\__('Parent Page ID'),
                    'parent.name' => \ProcessWire\__('Parent Page Name'),
                    'template.id' => \ProcessWire\__('Template ID'),
                    'template' => \ProcessWire\__('Template Name')
                ),
                ...
                "replaceModeNotes" => "WARNING: If you use this option, all the existing child \ProcessWire\pages(and grandchildren) will be deleted and new ones created.\n\nEach row is a separate page.\n\nYou can also use CSV formatted lines for populating all text/numeric fields on the page, eg:\n\"Bolivia, Plurinational State of\",BO,\"BOLIVIA, PLURINATIONAL STATE OF\",BOL,68",
    ...
    public function ready() {
        ...
        if(!$this->data['allowAdminPages'] && ($this->editedPage->template->flags & \ProcessWire\Template::flagSystem)) return;
    ...
    public function pageListCountPages(\ProcessWire\HookEvent $event) {
    ...
    public function buildPageConfigForm(\ProcessWire\HookEvent $event) {
        ...
        $fieldset->label = \ProcessWire\__("Batch Child Editor Settings");
        if(!isset($this->data['pageSettings'][$pp->id])) $fieldset->collapsed = \ProcessWire\Inputfield::collapsedYes;
            ...
            if($this->data['pageSettings'][$pp->id]['fieldPairings'] != '') {
                ...
                if(count($pp->template->childTemplates)==0) {
                    foreach($this->wire('templates') as $t) {
                        if(!($t->flags & \ProcessWire\Template::flagSystem)) $possibleTemplates[] = $t->name;
                    }
                }
                else {
    ...
    public function processPageConfigForm(\ProcessWire\HookEvent $event) {
    ...
    public function addInlineFieldset(\ProcessWire\HookEvent $event) {
    ...
    public function addChildEditFieldset(\ProcessWire\HookEvent $event) {
    ...
    public function addNewTab(\ProcessWire\HookEvent $event) {
        ...
        //if going back to this approach, then need to change the hook that calls this method to \ProcessWire\ProcessPageEdit::buildForm
        /*
        // create the tab
        $newTab = new \ProcessWire\InputfieldWrapper();
        ...
        */
        ...
        if(!$this->wire('input')->get->s) $field->collapsed = $this->currentData['openMethod'] == 'ajax' ? \ProcessWire\Inputfield::collapsedYesAjax : \ProcessWire\Inputfield::collapsedNo;
    ...
    public function createChildEditSet($pp, $settingsPage, $form) {
        ...
        foreach($pp->children("include=all") as $cp) {
            if(!$cp->is(\ProcessWire\Page::statusSystemID)) $this->childPages .= "{$cp->title}\r\n";
        }
        ...
        if($this->currentData['position'] != 'newTab' && !$this->wire('input')->get->s) $childEditSet->collapsed = $this->currentData['loadOpen'] ? '' : ((defined('Inputfield::collapsedYesAjax') || defined('\ProcessWire\Inputfield::collapsedYesAjax')) && $this->currentData['openMethod'] == 'ajax' ? \ProcessWire\Inputfield::collapsedYesAjax : \ProcessWire\Inputfield::collapsedYes);
        ...
        if(count($this->currentData['editModes'])>1) {
            $editModes = array(
                'lister' => \ProcessWire\__('Lister'),
                'edit' => \ProcessWire\__('Edit'),
                'add' => \ProcessWire\__('Add'),
                'update' => \ProcessWire\__('Update'),
                'replace' => \ProcessWire\__('Replace'),
                'export' => \ProcessWire\__('Export')
            );
            $f = $this->wire('modules')->get("InputfieldRadios");
            $f->attr('name', 'edit_mode');
            $f->label = \ProcessWire\__('Mode');
        ...
        if(is_array($pp->template->childTemplates) && count($pp->template->childTemplates)!=1) {
            ...
            $f->label = \ProcessWire\__('Child Template');
            $f->columnWidth = 34;
            $f->description = \ProcessWire\__('Choose the template for children');
            $f->notes = \ProcessWire\__('This will not change the template for existing \ProcessWire\pages(only for new children) in Update mode unless the "Update template for existing children" option is checked.');
            /*if(count($pp->template->childTemplates)==0) {
                foreach($this->wire('templates') as $t) {
                    if(!($t->flags & \ProcessWire\Template::flagSystem)) $f->addOption($t->name);
                }
            }
            else {
                $f->addOption('');
            ...
            $f->description = \ProcessWire\__('This will change the template for existing children to match the one selected.');
            $f->notes = \ProcessWire\__('This could destroy lots of content - use with caution!');
            $f->showIf = "edit_mode.count>0, edit_mode!=edit, edit_mode=update, edit_mode!=lister";
            $f->label = \ProcessWire\__('Update template for existing children');
            $f->columnWidth = 33;
        ...
        if($this->currentData['allowOverrideOverwriteNames']) {
            ...
            $f->label = \ProcessWire\__('Overwrite names');
            $f->columnWidth = 33;
            $f->description = \ProcessWire\__('Whether to overwrite the name of the page, and not just the title.');
            $f->attr('checked', $this->currentData['overwriteNames'] ? 'checked' : '' );
            $f->notes = \ProcessWire\__("This option can cause problems if the affected child pages are part of the front end structure of the site. It may result in broken links, etc.");
            $childEditSet->add($f);
        }
        ...
        if($this->wire('languages')) {
            ...
            $f->label = \ProcessWire\__('Activate New Pages in Other Languages');
            $f->columnWidth = 33;
            ...
            $f->showIf = "edit_mode.count>0, edit_mode!=edit, edit_mode!=export, edit_mode!=lister";
            $f->label = \ProcessWire\__('Choose the language to populate');
            $f->required = true;
            $f->columnWidth = 34;
            ...
            $f->label = \ProcessWire\__('Populate field content to all languages');
            $f->description = \ProcessWire\__('Only check this if this if the default language is the first language you are importing.');
            $f->columnWidth = 33;
            $childEditSet->add($f);
        }
        ...
        if((isset($this->currentData['allowOverrideCsvImportSettings']) && $this->currentData['allowOverrideCsvImportSettings']) && (in_array('add', $this->currentData['editModes']) || in_array('update', $this->currentData['editModes']) || in_array('replace', $this->currentData['editModes']))) {
            ...
            $csvSettingsFieldset->label = \ProcessWire\__("CSV Import Settings");
            ...
            if($fieldSeparator == $this->currentData['csvImportFieldSeparator'] && $fieldEnclosure == $this->currentData['csvImportFieldEnclosure'] && $ignoreFirstRow == $this->currentData['ignoreFirstRow']) $csvSettingsFieldset->collapsed = \ProcessWire\Inputfield::collapsedYes;
            $childEditSet->add($csvSettingsFieldset);

            $f = $this->wire('modules')->get("InputfieldText");
            $f->name = 'userCsvImportFieldSeparator';
            $f->label = \ProcessWire\__('CSV fields separated with');
            $f->showIf = "edit_mode=add|update|replace";
            $f->notes = \ProcessWire\__('For tab separated, enter: tab');
            ...
            $f->label = \ProcessWire\__('CSV field enclosure');
            $f->showIf = "edit_mode=add|update|replace";
            ...
            $f->label = \ProcessWire\__('CSV ignore the first row');
            $f->description = \ProcessWire\__('Use this if the first row contains column/field labels.');
            $f->showIf = "edit_mode=add|update|replace";
        ...
        if(in_array('add', $this->currentData['editModes'])) {
            ...
            if(is_array($this->currentData['csvOptions']) && in_array('paste', $this->currentData['csvOptions']) || !$this->currentData['csvOptions']) {
                ...
                $f->label = \ProcessWire\__("Text / Paste CSV");
                if($this->currentData['ignoreFirstRow']) $f->notes = \ProcessWire\__("If first row is 'Title' then the import settings were initially set to ignore first row of the entered data.");
                $f->attr('value', $this->currentData['ignoreFirstRow'] ? 'Title' : '');
                if(!$this->currentData['csvOptionsCollapsed'] || ($this->currentData['csvOptionsCollapsed'] && !in_array('paste',$this->currentData['csvOptionsCollapsed']))) $f->collapsed = \ProcessWire\Inputfield::collapsedYes;
            ...
            if(is_array($this->currentData['csvOptions']) && in_array('link', $this->currentData['csvOptions'])) {
                ...
                $f->name = "csvUrlAdd";
                $f->showIf = "edit_mode=add";
                $f->label = \ProcessWire\__("Enter URL to CSV file");
                $f->placeholder = \ProcessWire\__("URL to CSV file");
                $f->attr('value', '');
                if(!$this->currentData['csvOptionsCollapsed'] || ($this->currentData['csvOptionsCollapsed'] && !in_array('link',$this->currentData['csvOptionsCollapsed']))) $f->collapsed = \ProcessWire\Inputfield::collapsedYes;
                $csvFieldset->append($f);
            }

            if(is_array($this->currentData['csvOptions']) && in_array('upload', $this->currentData['csvOptions'])) {
                $f = $this->wire('modules')->get("InputfieldMarkup");
                $f->label = \ProcessWire\__("Upload CSV file");
                $f->name = 'csvUploadAdd';
                $f->showIf = "edit_mode=add";
                $f->value = "<input name='csvAddFile' type='file' />";
                if(!$this->currentData['csvOptionsCollapsed'] || ($this->currentData['csvOptionsCollapsed'] && !in_array('upload',$this->currentData['csvOptionsCollapsed']))) $f->collapsed = \ProcessWire\Inputfield::collapsedYes;
                $csvFieldset->add($f);
            }
        ...
        if(in_array('edit', $this->currentData['editModes'])) {
            ...
            $headerRow = array(
                \ProcessWire\__('Sort'),
                \ProcessWire\__('Title'),
                \ProcessWire\__('Name'),
                \ProcessWire\__('Template'),
                \ProcessWire\__('Hidden'),
                \ProcessWire\__('Unpublished'),
                \ProcessWire\__('View'),
                \ProcessWire\__('Edit'),
                \ProcessWire\__('Delete')
            );
            ...
            foreach ($pp->children("include=all") as $cp) {
                ...
                if ($cp->is(\ProcessWire\Page::statusSystemID)) continue; //Don't display if it is a system \ProcessWire\page(404, admin, trash etc)
                ...
                $row = array(
                    ...
                    '<span class="bcePageNameContainer"><span class="bcePageName" ' . $langInfo . ' id="name_'.$cp->id.'" style="' . ($cp->is(\ProcessWire\Page::statusUnpublished) ? 'text-decoration:line-through;' : '') . ($cp->is(\ProcessWire\Page::statusHidden) ? 'color:#8c8c8c;' : '') . '">'.$this->getLanguageVersion($cp, 'name', $userLang, true) . ($this->wire('modules')->isInstalled("LanguageSupportPageNames") && !$cp->localName($userLang) ? '</span>&nbsp;<i class="fa fa-info-circle" title="No name for '.$userLang->title.'" style="color:#cccccc; float:right" aria-hidden="true"></i>' : '') . '</span>',
                    ...
                    '<input id="hiddenStatus_'.$cp->id.'" class="hiddenStatus" name="hiddenStatus['.$cp->id.']" type="checkbox" '.($cp->is(\ProcessWire\Page::statusHidden) ? 'checked' : '').'/>',
                    '<input id="unpublishedStatus_'.$cp->id.'" class="unpublishedStatus" name="unpublishedStatus['.$cp->id.']" type="checkbox"'.($cp->is(\ProcessWire\Page::statusUnpublished) ? ' checked' : '').(!$allowPublish ? ' disabled' : '').'/>',
        ...
        if(in_array('lister', $this->currentData['editModes']) && $this->wire('user')->hasPermission('page-lister')) {
            ...
            $f->markupFunction = function($inputfield) use($pp) {
                ...
                $url = \ProcessWire\ProcessPageLister::addSessionBookmark($id, $bookmark) . '&modal=inline&minimal='.($this->currentData['listerConfigurable'] ? '0' : '1');
                if($url) return "
        ...
        if(in_array('export', $this->currentData['editModes']) && $pp->numChildren() > 0) {
            ...
            if($this->currentData['allowOverrideCsvExportSettings']) {
                ...
                $f->label = \ProcessWire\__('Pages to Include');
                $f->description = \ProcessWire\__('Leave blank to automatically select all child pages&nbsp;(not hidden and published)');
                ...
                $f->label = \ProcessWire\__('Fields to export');
                $f->description = \ProcessWire\__('Choose and sort the fields to include in the CSV export');
                ...
                $f->label = \ProcessWire\__('Columns separated with');
                $f->notes = \ProcessWire\__('For tab separated, enter: tab');
                ...
                $f->label = \ProcessWire\__('Column enclosure');
                ...
                $f->label = \ProcessWire\__('File extension');
                ...
                $f->label = \ProcessWire\__('Multiple values separator');
                $f->description = \ProcessWire\__('Separator for multiple values like Page fields, files/images, multiplier, etc.');
                $f->notes = \ProcessWire\__('Default is | Other useful options include \r for new lines when importing into Excel.');
                ...
                $f->label = \ProcessWire\__('Column labels');
                ...
                $f->label = \ProcessWire\__('Format Export');
                $f->label2 = \ProcessWire\__('Turns on output formatting for exported values.');
                $f->notes = \ProcessWire\__('If you will be importing this back into ProcessWire, you should leave unchecked.');
    ...
    public function saveChildren(\ProcessWire\HookEvent $event) {
        ...
        if($this->wire('input')->post->edit_mode == "replace") {
            ...
            if(!$this->currentData['disableContentProtection']) {
                $disableNote = $this->wire('user')->isSuperuser() ? \ProcessWire\__(" You can prevent this check by disabling Content Protection in the module config settings.") : '';
                foreach($pp->children("include=all") as $cp) {
                    if($cp->numChildren>0) {
        ...
        //Edit Mode
            ...
            foreach($this->wire('input')->post->individualChildTitles as $id => $childTitle) {
                ...
                if(strpos($id, 'new') !== FALSE) {
                    ...
                    if(isset($this->wire('input')->post->hiddenStatus[$id])) $cp->addStatus(\ProcessWire\Page::statusHidden);
                    if(isset($this->wire('input')->post->unpublishedStatus[$id])) $cp->addStatus(\ProcessWire\Page::statusUnpublished);
                }
                // existing page
                else {
                    ...
                    if(!is_null($this->wire('input')->post->hiddenStatus) &&
                        ...
                        $cp->addStatus(\ProcessWire\Page::statusHidden);
                    }
                    else {
                        $cp->removeStatus(\ProcessWire\Page::statusHidden);
                    }

                    if(!is_null($this->wire('input')->post->unpublishedStatus) &&
                        array_key_exists($cp->id, $this->wire('input')->post->unpublishedStatus)) {
                        $cp->addStatus(\ProcessWire\Page::statusUnpublished);
                    }
                    else {
                        $cp->removeStatus(\ProcessWire\Page::statusUnpublished);
                    }
                ...
                if($cp->isChanged('title') && ($this->wire('input')->post->userOverwriteNames || (!$this->wire('input')->post->userOverwriteNames && !$this->currentData['allowOverrideOverwriteNames'] && $this->currentData['overwriteNames']))) {
                    ...
                    $n = 0;
                    $pageName = $this->wire('sanitizer')->pageName($childTitle, \ProcessWire\Sanitizer::translate);
                    $matchedChildId = null;
                    do {
                        $name = $pageName . ($n ? "-$n" : '');
 ...
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/BatchChildEditor-master' . '/parsecsv-for-php/parsecsv.lib.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
        ...
        try {
            ...
            foreach($childPagesArray->data as $row) {
                ...
                if($x==1 && $this->wire('input')->post->edit_mode == "replace") {
                    foreach($pp->children("include=all") as $cp) {
                        if(!$cp->is(\ProcessWire\Page::statusSystemID) && $cp->trashable()) {
                ...
                if($i==0 && $this->currentData['fieldPairings'] != '') {
                    ...
                    if($this->wire('input')->post->edit_mode == "update") {
                        ...
                        if($this->wire('input')->post->userOverwriteNames || (!$this->wire('input')->post->userOverwriteNames && !$this->currentData['allowOverrideOverwriteNames'] && $this->currentData['overwriteNames'])) {
                            if($titleKey) $this->setPageTitleOrName('name', trim($this->wire('sanitizer')->pageName($row[$titleKey-1], \ProcessWire\Sanitizer::translate)), $np, $newPage);
                        }
                    }
                    ...
                    // else field pairings not defined, so match all fields in order
                        ...
                        if($i==0) {
                            ...
                            if($this->wire('input')->post->edit_mode == "update") {
                                ...
                                if($this->wire('input')->post->userOverwriteNames || (!$this->wire('input')->post->userOverwriteNames && !$this->currentData['allowOverrideOverwriteNames'] && $this->currentData['overwriteNames'])) {
                                    if($np && $np->id) {
                                        $this->setPageTitleOrName('name', trim($this->wire('sanitizer')->pageName($childFieldValue, \ProcessWire\Sanitizer::translate)), $np, $newPage);
                                    }
    ...
    private function populateField($np, $fieldName, $childFieldValue) {
        ...
        if($f->type instanceof \ProcessWire\FieldtypePage) {
        ...
        elseif($f->type instanceof FieldtypeMapMarker) {
            ...
            if($this->wire('languages') && $f->type instanceof \ProcessWire\FieldtypeLanguageInterface) {
                $langs = $this->wire('session')->populateToAllLanguages ? $this->wire('languages') : array($this->wire('session')->languageToPopulate);
                foreach($langs as $lang) {
    ...
    private function updateFileFields($f, $fieldsArrayItem, $np, $childFieldValue, $newImageFirst = false) {
        ...
        foreach(explode($importMultipleValuesSeparator, $childFieldValues) as $childFieldValue) {
            if(file_exists($childFieldValue) || strpos($childFieldValue,'//') !== false) {
                try {
                    $imageName = $this->wire('sanitizer')->pageName(pathinfo($childFieldValue, PATHINFO_BASENAME), \ProcessWire\Sanitizer::translate);
    ...
    private function createNewPageFieldPage($title, $options) {
        $newPageFieldPage = new \ProcessWire\Page();
    ...
    public function getModuleConfigInputfields(array $data) {
        ...
        $fieldset->label = \ProcessWire\__("Where editing tools are available and separately configurable");
        ...
        $fieldEnabledTemplates->label = \ProcessWire\__('Enabled templates', \ProcessWire\wire("config")->paths->root . 'site/modules/BatchChildEditor-master/BatchChildEditor.module.php');
        $fieldEnabledTemplates->columnWidth = 33;
        $fieldEnabledTemplates->description = \ProcessWire\__("The batch editing option will only be available for the selected templates.\nLeave blank to allow all templates.", \ProcessWire\wire("config")->paths->root . 'site/modules/BatchChildEditor-master/BatchChildEditor.module.php');
        $fieldEnabledTemplates->setAsmSelectOption('sortable', false);

        // populate with all available templates
        foreach($this->wire('templates') as $t) {
            // filter out system templates
            if(!($t->flags & \ProcessWire\Template::flagSystem)) $fieldEnabledTemplates->addOption($t->name);
        }
        ...
        $fieldEnabledPages->label = \ProcessWire\__('Enabled pages', \ProcessWire\wire("config")->paths->root . 'site/modules/BatchChildEditor-master/BatchChildEditor.module.php');
        $fieldEnabledPages->columnWidth = 34;
        $fieldEnabledPages->description = \ProcessWire\__("The batch editing option will only be available for the selected pages.\nLeave blank to allow all pages.", \ProcessWire\wire("config")->paths->root . 'site/modules/BatchChildEditor-master/BatchChildEditor.module.php');
        ...
        $fieldConfigurablePages->label = \ProcessWire\__('Configurable pages', \ProcessWire\wire("config")->paths->root . 'site/modules/BatchChildEditor-master/BatchChildEditor.module.php');
        $fieldConfigurablePages->columnWidth = 33;
        $fieldConfigurablePages->description = \ProcessWire\__("Selected pages will have their own config settings for Batch Child Editor - these will be added to the page's Settings tab. All other pages will use the config settings below.", \ProcessWire\wire("config")->paths->root . 'site/modules/BatchChildEditor-master/BatchChildEditor.module.php');
    ...
    protected function getAllowedTemplatesAdd($parent) {
        ...
        /*
        if($parent->hasStatus(\ProcessWire\Page::statusUnpublished)) {
            $parentEditable = $parent->editable();
        } else {
            ...
            $parent->addStatus(\ProcessWire\Page::statusUnpublished);
            $parentEditable = $parent->editable();
            $parent->removeStatus(\ProcessWire\Page::statusUnpublished);
     ...
     *
     */
    protected function getAllowedParentsAdd(\ProcessWire\Template $template = null, $pp) {
        if(count($this->predefinedParents)) {
            $parents = $this->predefinedParents;
        } else {
            $parents = new \ProcessWire\PageArray();
     ...
     *
     */
    protected function isAllowedTemplateAdd($template, \ProcessWire\Page $parent = null) {
        if(!is_object($template)) $template = $this->wire('templates')->get($template);
        if(!$template) throw new \ProcessWire\WireException('Unknown template');
    ...
    function buildCoreSettings ($data, $wrapper, $pid = null) {
        ...
        $fieldset->label = \ProcessWire\__("Mode Settings");
        $fieldset->collapsed = \ProcessWire\Inputfield::collapsedYes;
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldCheckboxes");
        $f->attr('name', 'editModes');
        $f->label = \ProcessWire\__('Modes');
        $f->required = true;
        $f->columnWidth = 25;
        $f->description = \ProcessWire\__('Which modes you want available. If more than one checked, then the user can decide.');
        if(defined('Inputfield::collapsedYesAjax') || defined('\ProcessWire\Inputfield::collapsedYesAjax')) $f->addOption("lister", \ProcessWire\__("Lister"));
        $f->addOption("edit", \ProcessWire\__("Edit"));
        $f->addOption("add", \ProcessWire\__("Add"));
        $f->addOption("update", \ProcessWire\__("Update"));
        $f->addOption("replace", \ProcessWire\__("Replace"));
        $f->addOption("export", \ProcessWire\__("Export CSV"));
        $f->value = $pid && isset($data['pageSettings'][$pid]) ? $data['pageSettings'][$pid]['editModes'] : $data['editModes'];
        $f->notes = \ProcessWire\__("The Edit and Add modes are safer than the Update and Replace modes, because they are non-destructive.\n\nUpdate will edit the content of fields of existing pages. Replace will delete all child pages and create new ones.");
        $fieldset->add($f);

        $f = $this->wire('modules')->get("InputfieldRadios");
        $f->attr('name', 'defaultMode');
        $f->label = \ProcessWire\__('Default Mode');
        $f->columnWidth = 25;
        $f->showIf = "editModes.count>1";
        $f->description = \ProcessWire\__('If more than one mode available to user, which one should be open by default.');
        $f->addOption("", \ProcessWire\__("None"));
        if(defined('Inputfield::collapsedYesAjax') || defined('\ProcessWire\Inputfield::collapsedYesAjax')) $f->addOption("lister", \ProcessWire\__("Lister"));
        $f->addOption("edit", \ProcessWire\__("Edit"));
        $f->addOption("add", \ProcessWire\__("Add"));
        $f->addOption("update", \ProcessWire\__("Update"));
        ...
        $f->notes = \ProcessWire\__("Only relevant for Edit and Update modes. This option can cause problems if the affected child pages are part of the front end structure of the site. It may result in broken links, etc.");
        $fieldset->add($f);

        $f = $this->wire('modules')->get("InputfieldCheckbox");
        $f->attr('name', 'allowOverrideOverwriteNames');
        $f->label = \ProcessWire\__('Allow user to change "Overwrite Names" setting');
        $f->showIf = "editModes=edit|update";
        $f->columnWidth = 25;
        $f->description = \ProcessWire\__('Whether an admin user can change the override option when doing a batch edit.');
        ...
        $f->notes = \ProcessWire\__("Only relevant for Edit and Update modes.");
        $fieldset->add($f);

        $fieldset = $this->wire('modules')->get("InputfieldFieldset");
        $fieldset->attr('id', 'batch_child_editor_data_protection_fieldset');
        $fieldset->label = \ProcessWire\__("Content / Deletion / Protection");
        ...
        if($this->wire('input')->get->id) {
            ...
            $f->columnWidth = 100;
            $f->description = \ProcessWire\__('If selected, the editable children will be from this parent instead.');
        ...
        $f->label = \ProcessWire\__('Disable content protection');
        $f->showIf = "editModes=replace|edit";
        $f->columnWidth = 25;
        $f->description = \ProcessWire\__('If checked, replace and edit modes will destructively delete children if they have field content and/or their own children. This is not different to normal ProcessWire behavior, but with this module it is very easy to delete important content, hence this check.');
        $f->notes = \ProcessWire\__("This can be extremely destructive, use with extreme caution!");
        ...
        $f->label = \ProcessWire\__('Allow template changes');
        $f->showIf = "editModes=edit";
        $f->columnWidth = 25;
        $f->description = \ProcessWire\__('If checked, edit mode will allow template changes (if user has the "page-template" permission). There are no warnings about data loss on template change, so be careful with this.');
        $f->notes = \ProcessWire\__("This can be extremely destructive, use with extreme caution!");
        ...
        if(!$this->wire('input')->get->id) {
            ...
            $f->label = \ProcessWire\__('Allow editing admin pages');
            $f->showIf = "editModes=edit";
            $f->columnWidth = 25;
            $f->description = \ProcessWire\__('If checked, this module will work on Admin pages.');
            $f->notes = \ProcessWire\__("This can be extremely destructive, use with extreme caution - it has the potential to break the entire PW install!");
        ...
        $f->label = \ProcessWire\__('Trash or Delete');
        $f->columnWidth = 25;
        $f->description = \ProcessWire\__('Whether you want deleted pages moved to the trash or permanently deleted.');
        $f->addOption("trash", \ProcessWire\__("Trash"));
        $f->addOption("delete", \ProcessWire\__("Delete"));
        ...
        $fieldset->label = \ProcessWire\__("Lister mode settings");
        ...
        $f->label = \ProcessWire\__('Default Sort Order');
        $options = array();
        ...
        $f->addOption(\ProcessWire\__('Ascending'), $options);
        $labels = $options;
        foreach($options as $name => $label) {
            $options["-$name"] = $label;
            unset($options[$name]);
        }
        $f->addOption(\ProcessWire\__('Descending'), $options);
        ...
        $f->label = \ProcessWire\__('Default Columns');
        $f->showIf = "editModes=lister";
        $f->description = \ProcessWire\__('Which columns displayed by default.');
        $f->addOption('title', \ProcessWire\__("Title"));
        ...
        $f->label = \ProcessWire\__('Default Filter');
        $f->showIf = "editModes=lister";
        $f->description = \ProcessWire\__('Which filters will be applied by default.');
        $f->notes = \ProcessWire\__('These filters will be in addition to limiting the results to the child of the current page.');
        ...
        $f->label = \ProcessWire\__('Allow Lister Configuration');
        $f->showIf = "editModes=lister";
        $f->description = \ProcessWire\__('If checked, the user will be able to configure the Filters, Columns, and use Actions.');
        ...
        $fieldset->label = \ProcessWire\__("Data entry / CSV import settings");
        ...
        $f->label = \ProcessWire\__('Page data options');
        $f->showIf = "editModes=add|update|replace";
        $f->required = true;
        $f->requiredIf = "editModes=add|update|replace";
        $f->columnWidth = $this->wire('input')->get->id ? 25 : 50;
        $f->description = \ProcessWire\__('Which page entry options do you want available.');
        $f->notes = \ProcessWire\__("For creating page title/names only, then Text / Paste is likely the most relevant option. CSV options to the right only relevant if populating other page fields in addition to the title/name.");
        $f->addOption("paste", \ProcessWire\__("Text / Paste"));
        $f->addOption("link", \ProcessWire\__("Link (URL)"));
        $f->addOption("upload", \ProcessWire\__("Upload"));
        $f->value = $pid && isset($data['pageSettings'][$pid]) ? $data['pageSettings'][$pid]['csvOptions'] : $data['csvOptions'];
        $fieldset->add($f);

        $f = $this->wire('modules')->get("InputfieldCheckboxes");
        $f->attr('name', 'csvOptionsCollapsed');
        $f->label = \ProcessWire\__('Page data options collapsed status');
        $f->showIf = "editModes=add|update|replace";
        $f->columnWidth = $this->wire('input')->get->id ? 25 : 50;
        $f->description = \ProcessWire\__('Which page entry options do you want open vs collapsed.');
        $f->notes = \ProcessWire\__("Checked options will be open by default.");
        $f->addOption("paste", \ProcessWire\__("Text / Paste"));
        $f->addOption("link", \ProcessWire\__("Link (URL)"));
        $f->addOption("upload", \ProcessWire\__("Upload"));
        ...
        if($this->wire('input')->get->id) {
            ...
            $f->label = \ProcessWire\__('CSV field pairings');
            $f->description = \ProcessWire\__("Enter CSV column number and Processwire field name pairs to match up when importing. If empty, all CSV columns will be imported in order and therefore must match the number of fields in the template for this page.");
            $f->notes = \ProcessWire\__("Column numbers start at 1 so in this example we are excluding the 3rd column in the CSV file:\n1:title\n2:body\n4:notes\n5:parentfield.subfield\n\nUnless the only modes are \"Edit\" and \"Update\", you must include at least the \"title\" field, but all other fields are optional.");
            $f->showIf = "editModes=add|update|replace";
            $f->columnWidth = 50;
            ...
            $f->label = \ProcessWire\__('Match children by first field pairing field');
            $f->showIf = "editModes=update";
            $f->columnWidth = 33;
            $f->description = \ProcessWire\__("If checked, child pages will be matched by the content of the first field pairing field, not the order of rows in the CSV file. If checked, the titles of existing child pages won't be shown in the Text/Paste input field.");
            $f->notes = \ProcessWire\__("Only relevant in Update mode. This is useful if you want to update only certain child pages. Note that you must have the field defined in the field pairings setting.");
            ...
            $f->label = \ProcessWire\__("Don't update case of matched field");
            $f->showIf = "editModes=update, matchByFirstField=1";
            $f->columnWidth = 34;
            $f->description = \ProcessWire\__('If checked, and the matched field was a case-insensitive match, do not update this field.');
            $f->notes = \ProcessWire\__("Only relevant in Update mode and if 'Match children by first field pairing field' is checked.");
            ...
            $f->label = \ProcessWire\__('Add new child page if no match');
            $f->showIf = "editModes=update, matchByFirstField=1";
            $f->columnWidth = 33;
            $f->description = \ProcessWire\__('If checked, a new child page will be created if the match by field returns no matching child page.');
            $f->notes = \ProcessWire\__("Only relevant in Update mode and if 'Match children by first field pairing field' is checked.");
        ...
        $f->label = \ProcessWire\__('Multiple values separator');
        $f->description = \ProcessWire\__('Separator for multiple values like Page fields, etc.');
        $f->notes = \ProcessWire\__('Default is | Other useful options include \r for new lines.');
        $f->showIf = "editModes=add|update|replace";
        ...
        $f->label = \ProcessWire\__('New Image First');
        $f->description = \ProcessWire\__('If new image added, add it in first place, rather than last.');
        $f->notes = \ProcessWire\__("Only relevant in Update mode.");
        $f->showIf = "editModes=update";
        ...
        $f->label = \ProcessWire\__('User override CSV settings');
        $f->description = \ProcessWire\__('Allow user to override CSV separator, enclosure, and ignore first row settings.');
        $f->showIf = "editModes=add|update|replace";
        ...
        $fieldset->label = \ProcessWire\__('CSV export settings');
        $fieldset->description = \ProcessWire\__('If you want to specify the fields to export, you need to define them separately for each parent page by setting the Configurable pages first. All these settings can also be overriden on a page specific basis.');
        ...
        $f->label = \ProcessWire\__('Column labels');
        $f->label2 = \ProcessWire\__('Put column names in the first row');
        $f->showIf = "editModes=export";
        ...
        $f->label = \ProcessWire\__('User override CSV settings');
        $f->description = \ProcessWire\__('Allow user to override Fields to export, CSV separator, enclosure, and column labels in first row settings.');
        $f->showIf = "editModes=export";
        ...
        $fieldset->label = \ProcessWire\__("Editor position, open, and load settings");
        $fieldset->collapsed = \ProcessWire\Inputfield::collapsedYes;
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldRadios");
        $f->attr('name', 'position');
        $f->label = \ProcessWire\__('Position');
        $f->columnWidth = 14;
        $f->description = \ProcessWire\__('Where to position the Batch Child Editor tool. Replace will remove the existing page tree. New Tab will put this editor in a new tab. Inline Fieldset will add the editor to the Content tab.');
        $f->addOption("top", \ProcessWire\__("Top"));
        $f->addOption("bottom", \ProcessWire\__("Bottom"));
        $f->addOption("replace", \ProcessWire\__("Replace"));
        $f->addOption("newTab", \ProcessWire\__("New Tab"));
        $f->addOption("inlineFieldset", \ProcessWire\__("Inline Fieldset"));
        ...
        $f->label = \ProcessWire\__('Insert After');
        $f->columnWidth = 14;
        $f->description = \ProcessWire\__('Choose the field you want to insert the edit fieldset after.');
        $f->notes = \ProcessWire\__('If none selected, it will be appended to the end of the Content tab.');
        foreach($this->wire('fields') as $field) {
        ...
        if($this->wire('input')->get->id) {
            ...
            $f->label = \ProcessWire\__('Hide Children');
            $f->columnWidth = 14;
            $f->description = \ProcessWire\__('If checked, all children will be removed from the page tree.');
            $f->notes = \ProcessWire\__('Use this option if you want to force editing of child pages through BCE.');
        ...
        $f->label = \ProcessWire\__('Remove Children Tab');
        $f->columnWidth = 14;
        $f->description = \ProcessWire\__('If checked, the Children tab will be removed.');
        $f->notes = \ProcessWire\__('This option to help to simplify the interface when the position is Inline Fieldset or New Tab');
        ...
        $f->label = \ProcessWire\__('Load open, not collapsed');
        $f->showIf = "position!=newTab";
        $f->columnWidth = 14;
        $f->description = \ProcessWire\__('If checked, batch child editor will initially be open, not collapsed.');
        ...
        if(defined('Inputfield::collapsedYesAjax') || defined('\ProcessWire\Inputfield::collapsedYesAjax')) {
            ...
            $f->label = \ProcessWire\__('Open Method');
            //$f->showIf = "loadOpen=0";
            $f->columnWidth = 14;
            $f->description = \ProcessWire\__('If initially collapsed, how should it be opened?');
            $f->notes = \ProcessWire\__('AJAX can be useful to improve performance of the page editor for page trees with a lot of children. Especially useful if you are using the "New Tab" position option.');
            $f->addOption("normal", \ProcessWire\__("Normal"));
            $f->addOption("ajax", \ProcessWire\__("AJAX"));
        ...
        $f = $this->wire('modules')->get("InputfieldText");
        $f->attr('name', 'tabName');
        $f->label = \ProcessWire\__('Tab / fieldset name');
        $f->required = 1;
        $f->columnWidth = 16;
        $f->description = \ProcessWire\__('Name for the tab / fieldset.');
        ...
        $fieldset->label = \ProcessWire\__("Custom labels, descriptions and notes");
        $fieldset->collapsed = \ProcessWire\Inputfield::collapsedYes;
        $wrapper->add($fieldset);

        $f = $this->wire('modules')->get("InputfieldText");
        $f->attr('name', 'editModeTitle');
        $f->label = \ProcessWire\__('Edit mode title');
        $f->showIf = "editModes=edit";
        $f->columnWidth = 33;
        $f->description = \ProcessWire\__('Custom title in Edit mode.');
        ...
        $f->label = \ProcessWire\__('Edit mode description');
        $f->showIf = "editModes=edit";
        $f->columnWidth = 34;
        $f->description = \ProcessWire\__('Custom text for description in Edit mode.');
        ...
        $f->label = \ProcessWire\__('Edit mode notes');
        $f->showIf = "editModes=edit";
        $f->columnWidth = 33;
        $f->description = \ProcessWire\__('Custom text for notes in Edit mode.');
        ...
        $f->label = \ProcessWire\__('Lister mode title');
        $f->showIf = "editModes=lister";
        $f->columnWidth = 33;
        $f->description = \ProcessWire\__('Custom title in Lister mode.');
        ...
        $f->label = \ProcessWire\__('Lister mode description');
        $f->showIf = "editModes=lister";
        $f->columnWidth = 34;
        $f->description = \ProcessWire\__('Custom text for description in Lister mode.');
        ...
        $f->label = \ProcessWire\__('Lister mode notes');
        $f->showIf = "editModes=lister";
        $f->columnWidth = 33;
        $f->description = \ProcessWire\__('Custom text for notes in Lister mode.');
        ...
        $f->label = \ProcessWire\__('Add mode title');
        $f->showIf = "editModes=add";
        $f->columnWidth = 33;
        $f->description = \ProcessWire\__('Custom title for Add mode.');
        ...
        $f->label = \ProcessWire\__('Add mode description');
        $f->showIf = "editModes=add";
        $f->columnWidth = 34;
        $f->description = \ProcessWire\__('Custom text for description in Add mode.');
        ...
        $f->label = \ProcessWire\__('Add mode notes');
        $f->showIf = "editModes=add";
        $f->columnWidth = 33;
        $f->description = \ProcessWire\__('Custom text for notes in Add mode.');
        $f->value = $pid && isset($data['pageSettings'][$pid]) ? $data['pageSettings'][$pid]['addModeNotes'] : $data['addModeNotes'];
        $fieldset->add($f);

        $f = $this->wire('modules')->get("InputfieldText");
        $f->attr('name', 'updateModeTitle');
        $f->label = \ProcessWire\__('Update mode title');
        $f->showIf = "editModes=update";
        $f->columnWidth = 33;
        $f->description = \ProcessWire\__('Custom title for Update mode.');
        ...
        $f->label = \ProcessWire\__('Update mode description');
        $f->showIf = "editModes=update";
        $f->columnWidth = 34;
        $f->description = \ProcessWire\__('Custom text for description in Update mode.');
        ...
        $f->label = \ProcessWire\__('Update mode notes');
        $f->showIf = "editModes=update";
        $f->columnWidth = 33;
        $f->description = \ProcessWire\__('Custom text for notes in Update mode.');
        ...
        $f->label = \ProcessWire\__('Replace mode title');
        $f->showIf = "editModes=replace";
        $f->columnWidth = 33;
        $f->description = \ProcessWire\__('Custom title for Replace mode.');
        ...
        $f->label = \ProcessWire\__('Replace mode description');
        $f->showIf = "editModes=replace";
        $f->columnWidth = 34;
        $f->description = \ProcessWire\__('Custom text for description in Replace mode.');
        ...
        $f->label = \ProcessWire\__('Replace mode notes');
        $f->showIf = "editModes=replace";
        $f->columnWidth = 33;
        $f->description = \ProcessWire\__('Custom text for notes in Replace mode.');
        ...
        $f->label = \ProcessWire\__('Export mode title');
        $f->showIf = "editModes=export";
        $f->columnWidth = 33;
        $f->description = \ProcessWire\__('Custom title for Export mode.');
        ...
        $f->label = \ProcessWire\__('Export mode description');
        $f->showIf = "editModes=export";
        $f->columnWidth = 34;
        $f->description = \ProcessWire\__('Custom text for description in Export mode.');
        ...
        $f->label = \ProcessWire\__('Export mode notes');
        $f->showIf = "editModes=export";
        $f->columnWidth = 33;
        $f->description = \ProcessWire\__('Custom text for notes in Export mode.');
