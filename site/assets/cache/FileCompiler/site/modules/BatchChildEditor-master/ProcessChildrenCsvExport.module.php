...
class ProcessChildrenCsvExport extends \ProcessWire\Process implements \ProcessWire\Module {
    ...
    public function exportCsv($event = NULL) {
        ...
        $defaultSettings = \ProcessWire\wire('modules')->get("BatchChildEditor")->getDefaultData();
        ...
        foreach($children as $p) {
            ...
            foreach($fieldNames as $fieldName) {
                ...
                */
                ...
                elseif(\ProcessWire\wire('fields')->$fieldName->type == "FieldtypeMapMarker") {
                    foreach(array('address', 'lat', 'lng', 'zoom', 'status') as $subFieldName) {
                        $values[] = $p->$fieldName->$subFieldName;
                    }
                    $formattedValue = implode($exportMultipleValuesSeparator, $values);
                }
                elseif(\ProcessWire\wire('fields')->$fieldName->type instanceof \ProcessWire\FieldtypeMulti && count($p->$fieldName) === 0) {
                    $formattedValue = '';
                }
                //All other fields
                else {
